/*
 * Copyright 2017-2020 NXP
 * All rights reserved.
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include "flexspi_nor_flash.h"
#include "fsl_flexspi_nor_boot.h"
#include "boards.h"

// Config for phyCORE MIMXRT1176 SoM flash (matches MicroPython settings)
#define FLASH_NOR_CONFIG_INIT                                                   \
  {                                                                             \
    .pageSize           = 256u,                                                 \
    .sectorSize         = 4u * 1024u,                                           \
    .ipcmdSerialClkFreq = kFlexSpiSerialClk_30MHz,                              \
    .blockSize          = 32u * 1024u,  /* 32KB block size */                   \
    .isUniformBlockSize = false,                                                \
    .memConfig          = {                                                     \
      .tag                = FLEXSPI_CFG_BLK_TAG,                                \
      .version            = FLEXSPI_CFG_BLK_VERSION,                            \
      .readSampleClkSrc   = kFlexSPIReadSampleClk_LoopbackFromDqsPad,           \
      .csHoldTime         = 5u,                                                 \
      .csSetupTime        = 5u,                                                 \
      .busyOffset         = 0u, /* Status bit 0 indicates busy. */              \
      .busyBitPolarity    = 0u, /* Busy when the bit is 1. */                   \
      .deviceModeCfgEnable = 1u,                                                \
      .deviceModeType     = kDeviceConfigCmdType_QuadEnable,                    \
      .deviceModeSeq      = {                                                   \
        .seqId  = 4u,                                                           \
        .seqNum = 1u,                                                           \
      },                                                                        \
      .deviceModeArg  = 0x02, /* QE bit for IS25WP series */                    \
      .deviceType     = kFlexSpiDeviceType_SerialNOR,                           \
      .sflashPadType  = kSerialFlash_4Pads,                                     \
      .serialClkFreq  = 7, /* 133MHz for RT1170 */                              \
      .sflashA1Size   = FLASH_SIZE,                                             \
      .lookupTable    = {                                                       \
        /* FLEXSPI_LUT_SEQ(cmd0, pad0, op0, cmd1, pad1, op1) */                 \
        /* The high 16 bits is command 1 and the low are command 0. */          \
        /* Within a command, the top 6 bits are the opcode, the next two...*/   \
                                                                                \
        /* Indices with ROM should always have the same function... */          \
                                                                                \
        /* 0: ROM: Read LUTs */                                                 \
        /* Quad version */                                                      \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0xEB /* the command to send */,                       \
                          RADDR_SDR, FLEXSPI_4PAD,                              \
                          24 /* bits to transmit */),                           \
          FLEXSPI_LUT_SEQ(DUMMY_SDR, FLEXSPI_4PAD,                              \
                          6 /* 6 dummy cycles... */,                            \
                          READ_SDR, FLEXSPI_4PAD, 0x04),                        \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 1: ROM: Read status */                                               \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x05 /* the command to send */,                       \
                          READ_SDR, FLEXSPI_1PAD, 0x02),                        \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 2: Empty */                                                          \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 3: ROM: Write Enable */                                              \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x06 /* the command to send */,                       \
                          STOP, FLEXSPI_1PAD, 0x00),                            \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 4: Config: Write Status Register (for QE bit) */                     \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x31 /* Write Status Register-2 */,                   \
                          WRITE_SDR, FLEXSPI_1PAD, 0x01),                       \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 5: ROM: Erase Sector */                                              \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x20 /* the command to send */,                       \
                          RADDR_SDR, FLEXSPI_1PAD,                              \
                          24 /* bits to transmit */),                           \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 6: Empty */                                                          \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 7: Empty */                                                          \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 8: Block Erase (32KB) */                                             \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x52 /* 32KB block erase */,                          \
                          RADDR_SDR, FLEXSPI_1PAD,                              \
                          24 /* bits to transmit */),                           \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 9: ROM: Page program */                                              \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x02 /* the command to send */,                       \
                          RADDR_SDR, FLEXSPI_1PAD,                              \
                          24 /* bits to transmit */),                           \
          FLEXSPI_LUT_SEQ(WRITE_SDR, FLEXSPI_1PAD,                              \
                          0x04 /* data out */,                                  \
                          STOP, FLEXSPI_1PAD, 0),                               \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 10: Empty */                                                         \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 11: ROM: Chip erase */                                               \
        SEQUENCE(                                                               \
          FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,                                \
                          0x60 /* the command to send */,                       \
                          STOP, FLEXSPI_1PAD, 0),                               \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS,                                                      \
          TWO_EMPTY_STEPS),                                                     \
                                                                                \
        /* 12: Empty */                                                         \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 13: ROM: Read SFDP */                                                \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 14: ROM: Restore no cmd */                                           \
        EMPTY_SEQUENCE,                                                         \
                                                                                \
        /* 15: ROM: Dummy */                                                    \
        EMPTY_SEQUENCE                                                          \
      },                                                                        \
    },                                                                          \
  }

__attribute__((section(".boot_hdr.conf"))) const flexspi_nor_config_t flash_nor_config = FLASH_NOR_CONFIG_INIT;

// For blhost image-load, to create ivt at zero address we cooks boot data and left-out fcfb section. We need a copy to
// write to flash.
const flexspi_nor_config_t flash_nor_config_copy = FLASH_NOR_CONFIG_INIT;
