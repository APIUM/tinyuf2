...
| Flash | OFF    | OFF    | ON     | OFF    | Boot from flash (normal operation) |
...
3. **Set boot switches to SDP mode**: **0-0-0-1** (only switch 4 ON)
...
6. **Set boot switches to Flash mode**: **0-0-1-0** (only switch 3 ON)
